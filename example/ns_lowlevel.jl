...
for i = 1:nsp, j = 1:nsp, k = 0:ny+1, l = 0:nx+1
    u0[:, i, j, k, l] .= prim_conserve([1.0, 0.0, 0.0, 1.0], gas.γ)
end
...
function KitBase.flux_gks!(
    ...
    @. fw +=
        Mt[4] * primL[1] * MuvL - 
        tau * Mt[4] * primL[1] * MauL -
        tau * Mt[4] * primL[1] * MauLT + 
        Mt[4] * primR[1] * MuvR -
        tau * Mt[4] * primR[1] * MauR - 
...
function dudt!(du, u, p, t)
    ...
    #=@inbounds for i = 1:nx
        for j = 1:ny, k = 1:nr, l = 1:ns
            ...
            fy[:, k, k, j, i] .= global_frame(fy[:, l, k, j, i], 0.0, 1.0) ./ ps.J[i, j][2]
        end
    end=#
    ...
    #=@inbounds for i = 1:nx
        for j = 0:ny+1, k = 1:nr, m = 1:4
            ...
            fy_face[m, 1, k, j, i] = dot(fx[m, :, k, j, i], ps.ll)
            fy_face[m, 2, k, j, i] = dot(fx[m, :, k, j, i], ps.lr)
        end
    end=#
    ...
    #=@inbounds for i = 1:nx
        for j = 1:ny+1, k = 1:nr
            ...
            uL = local_frame(uy_face[:, 2, k, j, i-1], 0.0, 1.0)
    ...
    @inbounds for i = 1:nx, j = 1:ny, k = 1:nr, l = 1:ns, m = 1:4
        ...
        #rhs2[m, l, k, j, i] = dot(fx[m, :, k, j, i], ps.dl[l, :])
    end
    ...
    @inbounds for i = 1:nx, j = 1:ny, k = 1:nr, l = 1:ns, m = 1:4
        du[m, l, k, j, i] =
            -(
                ...
                #(fy_interaction[m, k, j, i] - fy_face[m, 1, k, j, i]) * ps.dhl[l] +
                #(fy_interaction[m, k, j+1, i] - fy_face[m, 2, k, j, i]) * ps.dhr[l]
            )
    end
    du[:, :, :, :, 0] .= 0.0
    du[:, :, :, :, nx+1] .= 0.0
